...
// p.y*cos(a)}; } pt rot(pt p, double a) { return p * polar(1.0, a); } /// for
// complex pt rotate_to_b(pt a, pt b, lf ang) { return rot(a-b, ang)+b; }
...
// dot(a,b)/abs(a)/abs(b)))); } lf angle(pt a, pt b) { return atan2(cross(a, b),
// dot(a, b)); }
...
bool in_disk(pt a, pt b, pt p) { return dot(a - p, b - p) <= 0; }
...
struct polygon {
  ...
  void delete_repetead() {
    ...
    sort(p.begin(), p.end());
    for (pt &i : p)
      if (aux.empty() || aux.back() != i) aux.push_back(i);
    p.swap(aux);
  }
  bool is_convex() {
    bool pos = 0, neg = 0;
  ...
  vector<pii> antipodal() {
    ...
    for (int b = b0, a = 0; b != 0 && a <= b0; ++a) {
      ...
      if (abs(area2(p[a], p[nxt(a)], p[nxt(b)])) ==
          abs(area2(p[a], p[nxt(a)], p[b]))) {
        if (a != b0 || b != n - 1)
          ans.push_back({a, nxt(b)});
        else
          ans.push_back({nxt(a), b});
      }
    }
    return ans;
  }
